...
class Imap_mailbox extends CI_Model {
    ...
    public function fetchEmailsFromConfigs($limit) {
        ...
        $qFolders = $this->db->query("
                SELECT mailbox_configs_folders.*, COALESCE (maxes.date, NOW() - INTERVAL '5 years') AS last_mail_date, mailbox_configs_email
        ...
        foreach ($folders as $folder) {
            try {
                ...
                echo $ex->getMessage() . "- Email Address: {$folder['mailbox_configs_email']} <br/>";
                continue;
            }
            ...
            if ($emails) {
                // Lo faccio con una query unica che me le esclude
                $impMailId = "UNNEST(ARRAY['" . implode("','", $emails) . "'])";
                $query = $this->db->query("SELECT * FROM {$impMailId} AS item WHERE item NOT IN (SELECT mailbox_emails_external_id FROM mailbox_emails WHERE mailbox_emails_folder = ?)", [$folder['mailbox_configs_folders_id']]);
                $emails = array_key_map($query->result_array(), 'item');
                
                /*$strMailId = array_map(function($mailId) { return (string) $mailId; }, $emails);
                $this->db->select('mailbox_emails_external_id');
    ...
    protected function createMail($folderId, IncomingMail $email) {
        ...
        $strtotime = strtotime($email->date);
        if ($strtotime < 0) {
            $this->notices[] = "E-mail antecedente al 1970: skippata";
            return false;
